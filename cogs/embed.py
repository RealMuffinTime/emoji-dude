...
class Embed(commands.Cog):
    ...
    @commands.command(
        ...
        aliases=['commands', 'command', 'start'],
        usage='cog'
    )
        ...
        if cog == 'all':
            for cog in cogs:
                ...
                for comm in cog_commands:
                    commands_list += '**{comm.name}** - *{comm.description}*\n'
            ...
            if cog.lower() in lower_cogs:
                ...
                for command in commands_list:
                    ...
                    # Finally the format
                    help_text += 'Format: `@{self.bot.user.name}#{self.bot.user.discriminator}' \
                                 ' {command.name} {command.usage if command.usage is not None else ""}`\n\n\n\n'
    ...
    async def settings_command(self, ctx):
        await ctx.send(content='not programmed yet')
